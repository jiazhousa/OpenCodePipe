...
10. **消费者责任与两段式同步**（2026-09-17 用户裁决）：A 仓（规范）演进完全自由、不感知 B 仓；B 仓（实现）显式锁定 A 仓版本跟随（vendor-sync 拉取指定 tag/commit），同步责任在 B 仓。同步分两段——**机械同步**（vendor-sync 脚本，复制契约文件 + 更新哈希）与**语义适配**（涉语义变更必须开 B 仓 Story/Issue 走审查，哈希一致 ≠ 功能适配）；A 仓契约文件（07 卷 / templates）变更时 commit message 标注"B 仓需同步"（提示义务在 A 仓，执行义务在 B 仓）
